...
- 网站：coohom.com / kujiale.com
- 平台：Web / 客户端 / 小程序
- 定价：免费基础版，企业版按需定价
- 核心功能：户型图 → AI 智能布局 → 3D 渲染 → VR 全景漫游
- AI 功能：一键智能布局（上传户型图自动生成多套方案）、AI 灵感出图、照片/手绘转效果图
- 3D 预览：完整的 3D 实时预览 + 4K 渲染 + VR 漫游
- 优势：功能最全面，家具模型库海量，渲染质量高，国内市场占有率高
- 不足：定位偏 B 端（设计师/门店/工厂），消费者上手门槛高；输入依赖手动画户型图或上传图纸，不支持 LiDAR 扫描
- 差异：酷家乐是重型专业工具，RoomWise 做的是轻量消费级体验（手机扫描 → 浏览器看方案）
...
| 产品 | LiDAR 扫描 | 手动编辑 | AI 自动布局 | 3D 预览 | LLM 约束 | 轻量级 |
|------|-----------|---------|------------|--------|---------|--------|
| RoomWise | ✅ | 🚧 | ✅ 模拟退火 | 🚧 | 🚧 | ✅ Web |
| 酷家乐 | ❌ | ✅ | ✅ 一键布局 | ✅ 4K+VR | ❌ | ❌ 重型 |
| Planner 5D | ❌ | ✅ | ✅ Smart Wizard | ✅ | ❌ | ⚠️ 中等 |
| magicplan | ✅ | ✅ | ❌ | ⚠️ 基础 | ❌ | ✅ |
| Room Planner | ✅ | ✅ | ❌ | ✅ | ❌ | ✅ |
| RoomScan Pro | ✅ | ❌ | ❌ | ❌ | ❌ | ✅ |
| Live Home 3D | ✅ | ✅ | ❌ | ✅ | ❌ | ❌ 重型 |
...
核心不是扫描（扫描是前置步骤），核心是**自动布局引擎 + 交互式编辑**。

与酷家乐/Planner 5D 的区别：
- 酷家乐面向设计师和门店，RoomWise 面向普通消费者
- 酷家乐需要手动画户型图，RoomWise 手机扫一下就有
- 酷家乐的 AI 布局偏模板化（按户型套方案），RoomWise 基于约束求解，理解用户现有家具和个性化需求

演进方向：
1. **Phase 0（当前）**：扫描 → 2D 自动布局 → 多方案切换
2. **Phase 1**：2D/3D 手动编辑 + 锁定家具 + rescan
3. **Phase 2**：LLM 理解自然语言约束（"电视对着沙发"、"书桌靠窗"）
4. **Phase 3**：3D 渲染预览 + AR 投影 + 家具品牌模型
